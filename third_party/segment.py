...
From Autoencoded Vocal Analysis (AVA), https://github.com/pearsonlab/autoencoded-vocal-analysishttps://github.com/pearsonlab/autoencoded-vocal-analysis
...
P = {
    ...
    "min_dur": 1.0,  # minimum syllable duration
...
def segment(audio_dir, seg_dir, p, verbose=True):
    ...
    for audio_fn, seg_fn in zip(audio_fns, seg_fns):
        ...
        unique_ons = np.unique(onsets)
        unique_offs = np.unique(onsets)
        if len(onsets) != len(offsets):
            print("some onsets are duplicated!! how did this happen!")
            print(onsets, unique_ons)
            assert False
        if len(offsets) != len(unique_offs):
            print("some offsets are duplicated!! how did this happen!")
            print(offsets, unique_offs)
            assert False
...
def tune_segmenting_params(audio_dirs, p, img_fn="temp.pdf"):
    ...
    for load_dir in audio_dirs:
        filenames += [
            os.path.join(load_dir, i) for i in os.listdir(load_dir) if _is_audio_file(i)
        ]
    if len(filenames) == 0:
    ...
    while True:
        ...
        while temp != "s" and temp != "r":
            ...
            spec, dt, f = get_spec_ava(audio, p)

            # Get onsets and offsets.
            onsets, offsets, traces = p["algorithm"](audio, p, return_traces=True)
...
def get_audio_seg_filenames(audio_dir, segment_dir, p=None):
    ...
    """
    temp_filenames = [i for i in sorted(os.listdir(audio_dir)) if _is_audio_file(i)]
